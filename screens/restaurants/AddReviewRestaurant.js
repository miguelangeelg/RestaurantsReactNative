...
export default function AddReviewRestaurant({ navigation, route }) {
    ...
    const addReview = async() => {
        ...
        if (!responseGetRestaurant.statusResponse) {
            setLoading(false)
            toastRef.current.show("Error al obtener el espacio deportivo, por favor intenta más tarde.", 3000)
            return
        }
        ...
        if (!responseUpdateRestaurant.statusResponse) {
            toastRef.current.show("Error al actualizar el espacio deportivo, por favor intenta más tarde.", 3000)
            return
        }
    ...
    const validForm = () => {
        ...
        if (!rating) {
            toastRef.current.show("You may give the restaurant score", 3000)
            isValid = false
        }

        if (isEmpty(title)) {
            setErrorTitle("Yoy may enter a reviews title.")
            isValid = false
        }

        if (isEmpty(review)) {
            setErrorReview("You may enter a review.")
            isValid = false
        }
